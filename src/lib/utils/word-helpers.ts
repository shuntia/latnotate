...
/**
 * Get the case of a word if it's already been selected
 * This allows heuristics to respect existing selections
 */
export const getSelectedCase = (word: SentenceWord): string | null => {
  if (!word.selectedEntry || !word.selectedMorphology) return null;
  
  const cgn = getCaseGenderNumber(word.selectedMorphology);
  return cgn ? cgn.case : null;
};

/**
 * Filter lookup results to only show entries matching a specific case
 * Used to constrain search when word already has case selected
 */
export const filterByCaseIfSelected = (word: SentenceWord): WordEntry[] => {
  const selectedCase = getSelectedCase(word);
  if (!selectedCase || !word.lookupResults) return word.lookupResults || [];
  
  // If word already has a case, only return entries that can be that case
  return word.lookupResults.filter(entry => {
    return entry.morphologies.some(m => {
      const analysis = m.analysis || m.line || "";
      return analysis.includes(selectedCase);
    });
  });
};
...
export const isPotentialPreposition = (word: SentenceWord): boolean => {
  ...
  return word.lookupResults.some(
    (entry) =>
      entry.type === "Other" &&
 ...
 */
export const getSelectedPrepositionCases = (
  word: SentenceWord,
): string[] | null => {
