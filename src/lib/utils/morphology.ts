...
export const sortMorphologies = (
  morphologies: Morphology[],
  guessedAnalysis?: string,
) => {
  return morphologies.slice().sort((a, b) => {
    ...
    const numberCompare =
      getNumberOrder(a.analysis) - getNumberOrder(b.analysis);
    ...
    const personCompare =
      getPersonOrder(a.analysis) - getPersonOrder(b.analysis);
    return personCompare;
  });
};

export const formatMorphologyDisplay = (
  morphologies: Morphology[],
): Map<string, string> => {
