...
import { cleanWord, isAdjectival } from "@/lib/utils/word-helpers";
...
export const applyNominativeChunkGuessing = (words: SentenceWord[]): void => {
  ...
  for (let i = firstVerbIdx - 1; i >= 0; i--) {
    ...
    if (
      words[i].selectedEntry?.type === "Verb" &&
      words[i].selectedMorphology
    ) {
      sentenceStart = i + 1;
      break;
    }
  ...
  for (let i = sentenceStart; i <= sentenceEnd; i++) {
    ...
    // Skip if already selected and it's NOT nominative
    if (word.selectedEntry && word.selectedMorphology) {
      const cgn = getCaseGenderNumber(word.selectedMorphology);
      if (cgn && cgn.case !== "Nominative") continue;
    }
