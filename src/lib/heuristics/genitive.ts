...
export const applyGenitiveHeuristic = (words: SentenceWord[]): void => {
  words.forEach((word, idx) => {
    ...
    const shouldBlockTraversal = (w: SentenceWord): boolean => {
      ...
      if (!cleaned || cleaned === ',' || cleaned === '.' || cleaned === ';' || cleaned === '?' || cleaned === ':') return true;
      
      // Block on quotes (sentence separators)
      if (w.original === '"' || w.original === "'" || w.original === '«' || w.original === '»') return true;
      ...
      if (['qui', 'quae', 'quod', 'quem', 'quam', 'quo', 'qua', 'cuius', 'cui', 'quorum', 'quarum', 'quibus', 'quos', 'quas'].includes(cleaned)) {
        return true;
      }
