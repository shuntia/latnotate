...
export const applyAccusativeInfinitiveHeuristic = (words: SentenceWord[]): void => {
  // Find ACI verbs
  words.forEach((verb, verbIdx) => {
    ...
    for (let i = verbIdx + 1; i < Math.min(words.length, verbIdx + 10); i++) {
      ...
      for (let j = verbIdx + 1; j < i; j++) {
        ...
        if (!accSubject.selectedEntry || !accSubject.selectedMorphology) continue;
        ...
        const isPronoun = accSubject.selectedEntry.type === "Other" && 
