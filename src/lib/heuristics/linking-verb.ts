...
export const applyLinkingVerbHeuristic = (words: SentenceWord[]): void => {
  words.forEach((verb, verbIdx) => {
    ...
    const isLinkingVerb = verbForms.some(form => LINKING_VERB_FORMS.has(form));
    ...
    for (let i = Math.max(0, verbIdx - 5); i < Math.min(words.length, verbIdx + 5); i++) {
      if (i === verbIdx) continue;
      
      const candidate = words[i];
      if (!candidate.selectedEntry || !candidate.selectedMorphology) continue;
      if (candidate.selectedEntry.type !== "Noun" && candidate.selectedEntry.type !== "Adjective") continue;
    ...
    const searchEnd = subjectIdx < verbIdx ? Math.min(words.length, verbIdx + 5) : verbIdx;
    ...
    for (let i = searchStart; i < searchEnd; i++) {
      ...
      if (predCgn.gender === subjectCgn.gender && predCgn.number === subjectCgn.number) {
