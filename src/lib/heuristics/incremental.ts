...
import { isConjunction, getSelectedPrepositionCases } from "@/lib/utils/word-helpers";
...
export const applyIncrementalHeuristics = (words: SentenceWord[], changedIndex: number): void => {
        ...
        for (const entry of entries) {
          ...
          if (matchingMorphs.length === 1) {
            ...
            if (cgn) {
              ...
              const existingBracket = changedWord.annotations.find(a => a.type === "preposition-scope");
              if (!existingBracket && !changedWord.rejectedHeuristics?.has(`preposition-scope-${endIdx}`)) {
...
export const rerunDependentHeuristics = (words: SentenceWord[], changedIndex: number): void => {
  const changedWord = words[changedIndex];
  ...
  dependentIndices.forEach(depIdx => {
    ...
    const cgn = depWord.selectedEntry && depWord.selectedMorphology 
      ? getCaseGenderNumber(depWord.selectedMorphology) 
      : null;
    
    // If it's a genitive noun, rerun genitive heuristic
    if (cgn && cgn.case === "Genitive" && depWord.selectedEntry?.type === "Noun") {
