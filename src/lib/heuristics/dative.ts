...
const DATIVE_VERBS = new Set([
  ...
  "persuadeo", "persuadere", "persuasi", "persuasum", // persuade
  "impero", "imperare", "imperavi", "imperatum", // command
  "noceo", "nocere", "nocui", "nocitum", // harm
  "pareo", "parere", "parui", "paritum", // obey
  "placeo", "placere", "placui", "placitum", // please
  "servio", "servire", "servivi", "servitum", // serve
  "studeo", "studere", "studui", // be eager
  "faveo", "favere", "favi", "fautum", // favor
]);

export const applyDativeIndirectObjectHeuristic = (words: SentenceWord[]): void => {
  ...
  words.forEach((verb, verbIdx) => {
    ...
    for (let i = Math.max(0, verbIdx - 8); i < Math.min(words.length, verbIdx + 8); i++) {
      ...
      const isPronoun = candidate.selectedEntry.type === "Other" && 
