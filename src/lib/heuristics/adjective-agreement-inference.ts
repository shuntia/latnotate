import { SentenceWord, WordEntry, Morphology } from "@/lib/types/sentence";
 ...
 */
export const applyAdjectiveAgreementInference = (
  words: SentenceWord[],
): void => {
  // Helper to check if a word is inside a preposition scope (bracket)
  const isInPrepositionalPhrase = (wordIdx: number): boolean => {
    // Check all words for preposition-scope annotations
    for (const word of words) {
      for (const ann of word.annotations) {
        if (ann.type === "preposition-scope") {
          if (ann.targetIndex !== undefined && ann.endIndex !== undefined) {
            // Check if wordIdx is in the range [targetIndex, endIndex]
  ...
  words.forEach((word, wordIdx) => {
    ...
    const canBeAdjective = word.lookupResults.some((entry) =>
      isAdjectival(entry),
    );
    ...
    for (let offset = -3; offset <= 3; offset++) {
      ...
      let matchingAdjEntry: WordEntry | undefined = undefined;
      let matchingAdjMorph: Morphology | undefined = undefined;
      let matchingCase = "";
      let matchingGender = "";
      let matchingNumber = "";
      ...
      // Apply heuristic
      if (!matchingAdjEntry || !matchingAdjMorph) continue;

      word.selectedEntry = matchingAdjEntry;
      word.selectedMorphology =
        matchingAdjMorph.analysis || matchingAdjMorph.line;
      word.heuristic = `Inferred adjective agreeing with "${other.original}" (${matchingCase} ${matchingGender} ${matchingNumber})`;

      // Skip connection if the noun is in a prepositional phrase with ACC/ABL
      let skipConnection = false;
      if (isInPrepositionalPhrase(otherIdx) && other.selectedMorphology) {
        const otherCGN = getCaseGenderNumber(other.selectedMorphology);
        if (otherCGN && (otherCGN.case === "Accusative" || otherCGN.case === "Ablative")) {
          skipConnection = true;
        }
      }

      // Add modify annotation (unless skipped)
      if (!skipConnection) {
