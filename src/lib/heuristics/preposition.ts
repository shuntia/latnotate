...
function getPrepositionCases(word: SentenceWord): string[] {
  ...
  const isPrep = word.selectedEntry.type === "Other" && 
                 (word.selectedMorphology.includes("Preposition") || word.selectedMorphology.includes("PREP"));
 ...
 * Treats participles as adjectives
 */
function selectBestMorphology(word: SentenceWord, targetCase: string): { entry: any; morphology: string } | null {
  ...
  words.forEach((word, idx) => {
    ...
    for (const entry of prepEntries) {
      for (const morph of entry.morphologies) {
        ...
        if (!analysis.includes("Preposition") && !analysis.includes("PREP")) continue;
        
        // Match both expanded ("Accusative") and abbreviated ("ACC") forms
        if (analysis.match(/(Accusative|ACC)/i)) allPossibleCases.add("Accusative");
    ...
    if (idx + 1 < words.length) {
      ...
      // Check which cases the next word MUST be (if any)
      const nextGuaranteedCases = new Set<string>();
...
export const applyPrepositionInference = (words: SentenceWord[]): void => {
  words.forEach((word, idx) => {
    ...
    const viableOptions: Array<{ case: string; morphology: string; entry: any }> = [];
 ...
 */
export const applyPrepositionalObjectSelection = (words: SentenceWord[]): void => {
  words.forEach((word, idx) => {
    ...
    for (let i = idx + 1; i < Math.min(idx + 8, words.length); i++) {
      ...
      let bestMatch: { case: string; entry: any; morphology: string } | null = null;
...
export const applyPrepositionalBrackets = (words: SentenceWord[]): void => {
  words.forEach((word, idx) => {
    ...
    for (let i = objectIndex + 1; i < Math.min(objectIndex + 8, words.length); i++) {
      const candidate = words[i];
