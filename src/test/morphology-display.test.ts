...
function formatMorphologyDisplay(
  morphologies: Array<{ analysis: string }>,
): Map<string, string> {
  ...
  for (const { original, parts } of parsed) {
    ...
    for (const part of originalParts) {
      if (
        !seenParts.has(part) &&
        (!commonParts.has(part) || fieldsToKeep.has(part))
      ) {
        keptParts.push(part);
        seenParts.add(part);
      }
...
describe("Morphology Display Formatting", () => {
  it("should remove common gender from noun morphologies", () => {
    ...
    expect(result.get("Noun Nominative Singular Feminine")).toBe(
      "Nominative Singular",
    );
    expect(result.get("Noun Genitive Singular Feminine")).toBe(
      "Genitive Singular",
    );
    expect(result.get("Noun Dative Singular Feminine")).toBe("Dative Singular");
    expect(result.get("Noun Accusative Singular Feminine")).toBe(
      "Accusative Singular",
    );
    expect(result.get("Noun Ablative Singular Feminine")).toBe(
      "Ablative Singular",
    );
  });
  ...
  it("should keep number when it varies", () => {
    ...
    expect(result.get("Noun Nominative Plural Feminine")).toBe(
      "Nominative Plural",
    );
  });
  ...
  it("should put case first for nouns", () => {
    ...
    expect(result.get("Noun Accusative Singular Masculine")).toBe(
      "Accusative Singular",
    );
    expect(result.get("Noun Accusative Plural Masculine")).toBe(
      "Accusative Plural",
    );
  });
  ...
  it("should put person first for verbs", () => {
    ...
    expect(
      result.get("Verb Present Active Indicative 1st Person Singular"),
    ).toContain("1st");
    expect(
      result.get("Verb Present Active Indicative 2nd Person Singular"),
    ).toContain("2nd");
    expect(
      result.get("Verb Present Active Indicative 3rd Person Singular"),
    ).toContain("3rd");
  });
  ...
  it("should keep voice when it varies", () => {
    ...
    expect(
      result.get("Verb Present Passive Indicative 3rd Person Singular"),
    ).toContain("Passive");
  });
  ...
  it("should handle adjectives with varying cases", () => {
    ...
    expect(result.get("Adjective Positive Nominative Singular Masculine")).toBe(
      "Nominative Singular Positive",
    );
    expect(result.get("Adjective Positive Genitive Singular Masculine")).toBe(
      "Genitive Singular Positive",
    );
    expect(result.get("Adjective Positive Dative Singular Masculine")).toBe(
      "Dative Singular Positive",
    );
  });

  it("should handle single morphology entry", () => {
    const morphologies = [{ analysis: "Preposition Ablative" }];
